...
begin
  ...
  g := green(colorin);
  b := blue(colorin);
   par1 := 0.001;//Anziehung zum Zentrum
    par2 := 0.05;//Abstoßung von anderen Boids
    par3 := 0.001;//Näherung von Geschwindigkeit
    par4 := 0.001;//Antziehung zum Ziel
    mdist := 0.5;
    velCap := 0.6;
    dampening:=1;
...
begin
  ...
  begin
    ...
    begin
      ...
      zwsch2:= vecSub(inVec, boidsarr[i].pos);
      l:=vecLength(zwsch2);
      if(l>0)then
      zwsch := vecAdd(zwsch, vecDiv(zwsch2,l));
    end;
  end;
  rule2 := zwsch;
end;
...
begin
  center := findCenter();
  avVel := findAvrgVel();
  //center:=vecAdd(center,vecMul(vecSub(aim,center),-par4));
  for i := 0 to length(boidsarr) do
  begin
    ...
    boidsarr[i].vel := capVec(vecMul(vecAdd(boidsarr[i].vel, zwsch),dampening), velCap);
  end;
end;
